...
mod2 <- glm(nclaims ~ offset(log(expo))  + use + fuel + power +sex, 
           data = beMTPL97,
           family=poisson(link = log))
...
phi_pearson <- sum(residuals(mod2, type = "pearson")^2) / mod2$df.residual
phi_pearson

# A partir de la déviance
phi_dev <- deviance(mod2) / mod2$df.residual
phi_dev
